...
interface CuentaMovimientoRepository : JpaRepository<CuentaMovimiento?, Long?> {
    ...
    fun findAllByFechaAndOrden(
            fechaContable: OffsetDateTime,
            ordenContable: Int
    ): List<CuentaMovimiento>
    fun findAllByFechaBetween(
    ...
    fun findFirstByFechaAndOrdenOrderByItemDesc(
            fecha: OffsetDateTime?,
            orden: Int?
    ): Optional<CuentaMovimiento?>?
    fun findFirstByFechaOrderByOrdenDesc(fecha: OffsetDateTime?): Optional<CuentaMovimiento?>?

    @Query(
            "SELECT COALESCE(SUM(cm.importe), 0) FROM CuentaMovimiento cm WHERE cm.fecha BETWEEN :desde AND :hasta AND cm.numeroCuenta = :numeroCuenta AND cm.debita = :debita"
    )
    fun calculateTotalByNumeroCuentaAndDebitaAndFechaBetween(
            numeroCuenta: Long,
            debita: Int,
    ...
    @Query(
            "SELECT COALESCE(SUM(cm.importe), 0) FROM CuentaMovimiento cm WHERE cm.fecha BETWEEN :desde AND :hasta AND cm.numeroCuenta = :numeroCuenta AND cm.debita = :debita AND cm.inflacion = :inflacion"
    )
    ...
    fun findAllByClienteIdAndComprobanteIdAndConceptoLike(
            clienteId: Long,
            comprobanteId: Int,
            concepto: String
    ): List<CuentaMovimiento>
