...
public class ProgramaDiaService {
    ...
    // public List<ProgramaDiaDetallesDto> getProgramaDiaDetalles(OffsetDateTime
    // fechaServicio) {
    // List<ProgramaDiaDetallesDto> programas = new ArrayList<>();
    // List<Voucher> vouchers = voucherService.findAllByFechaServicio(fechaServicio,
    // false, false);
    ...
    // voucherProductoService.findAllByVoucherId(voucher.getVoucherId());
    ...
    // voucherProducto ->
    // articuloService.findAllByProductoId(voucherProducto.getProducto().getProductoId())));
    // // List<Producto> productos =
    // productoService.findAllByVoucherId(voucher.getVoucherId());
    // // Map<Integer, List<Articulo>> articulosByProducto = productos.stream()
    // // .collect(Collectors.toMap(
    // // Producto::getProductoId,
    // // producto ->
    // articuloService.findAllByProductoId(producto.getProductoId())));
    // List<Grupo> grupos =
    // grupoService.findAllByVoucherFechaServicio(fechaServicio);
    ...
    // grupoService.totalVentasByGrupoAndVoucher(g.getGrupoId(),
    // voucher.getVoucherId());

    // if (totalVentas == null || totalVentas.compareTo(BigDecimal.ZERO) == 0) {
    // return null;
    // }
    // return new VentasPorGrupoDto(fechaServicio, grupoToDtoMapper.toDto(g,
    // productos, articulosByProducto), totalVentas, null);
    // })
                ...
                .map(voucher -> {
                    ...
                    List<VentasPorGrupoDto> ventasPorGrupo = grupos.stream()
                            .map(g -> {
                                ...
                                BigDecimal totalVentas = grupoService.totalVentasByGrupoAndVoucher(g.getGrupoId(),
                                        voucher.getVoucherId());
                                ...
                                Map<Integer, List<Articulo>> grupoArticulosByProducto = productos.stream()
                                        ...
                                        .collect(Collectors.toMap(
                                                ...
                                                producto -> globalArticulosByProducto
                                                        .getOrDefault(producto.getProductoId(), new ArrayList<>())));
